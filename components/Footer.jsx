...
const Footer = ({ activeRoute = "home" }) => {
    ...
    const avatarOptions = {
        color: colors.color2, size: 50, style: { backgroundColor: colors.color1 }
    };

    return (
        <View style={{ backgroundColor: colors.color1, borderTopRightRadius: 120, borderTopLeftRadius: 120, position: "absolute", width: "100%", bottom: 0 }}>
            <View style={{ flexDirection: "row", justifyContent: "space-evenly" }}>
                ...
                <TouchableOpacity activeOpacity={0.8} onPress={() => navigationHandler(2)}>
                    <Avatar.Icon {...avatarOptions} icon={isAuthenticated === false ? "login" : activeRoute === "profile" ? "account" : "account-outline"} />
                </TouchableOpacity>
            </View>

            <View style={{ position: "absolute", width: 80, height: 80, backgroundColor: colors.color2, borderRadius: 180, justifyContent: "center", alignItems: "center", top: -50, alignSelf: "center" }}>
